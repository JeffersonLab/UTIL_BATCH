...
if [ ! -f "$REPLAYPATH/ROOTfiles/Calib/General/Pion_coin_replay_calibration_$RUNNUMBER_$MAXEVENTS.root" ]; then
    if [[ "${HOSTNAME}" != *"ifarm"* ]]; then
	eval "$REPLAYPATH/hcana -l -q \"UTIL_PION/scripts/replay/PionLT/replay_Cer_Calib_ALL.C($RUNNUMBER,$MAXEVENTS)\"" 
...
sleep 15
#cd "$REPLAYPATH/CALIBRATION/shms_aero_calib"
#echo "root -l -b -q \"paero_calib.C(\"Pion_coin_replay_calibration_${RUNNUMBER}_${MAXEVENTS}\")\""
#root -l -b -q "paero_calib.C(\"Pion_coin_replay_calibration_${RUNNUMBER}_${MAXEVENTS}\")" 
#sleep 15
